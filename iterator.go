...
func iterate(settings *Settings, config *Config) {
  ...
  // Shift rotor 1
  settings.Rotors[0].Position = incrementRotor(settings.Rotors[0].Position, config)

  // Shift rotor 2
  if shiftRotorTwo  {
    settings.Rotors[1].Position = incrementRotor(settings.Rotors[1].Position, config)
  }

  // Shift rotor 3
  if shiftRotorThree {
    settings.Rotors[2].Position = incrementRotor(settings.Rotors[2].Position, config)
  }
}

func incrementRotor(letter string, config *Config) string{
